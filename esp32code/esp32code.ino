...
#define WITHOUT_BIKE_RANDOM_GENERATOR
...
#ifdef WITHOUT_BIKE_RANDOM_GENERATOR
void generateRandomAndSendPacket(int r){
  // Prepare the buffer for the packet
  String bufferStr;
  bufferStr.concat("VRADA;");
  bufferStr.concat(PredefinedReceivers[0].c_str());
...
void loop() {
  #ifdef WITHOUT_BIKE_RANDOM_GENERATOR
    int r = random(1,100);
    Serial.println("Generating random integer and send the packet....");
    generateRandomAndSendPacket(r);
  #endif
  for (int i = 0; i < connectedClients; i++) {
